...
use conjugatepoc_runtime::{self, GenesisConfig, opaque::Block, RuntimeApi, AccountId};
...
native_executor_instance!(
	pub Executor,
	conjugatepoc_runtime::api::dispatch,
	conjugatepoc_runtime::native_version,
);
...
pub fn conjugatepoc_inherent_data_providers(author: Option<&str>) -> Result<inherents::InherentDataProviders, ServiceError> {
	...
	if !inherent_data_providers.has_provider(&srml_anyupgrade::INHERENT_IDENTIFIER) {
		...
		// ```
		// 	srml_anyupgrade::Call::<conjugatepoc_runtime::Runtime>::any(
...
macro_rules! new_full_start {
	($config:expr, $author:expr) => {{
		let inherent_data_providers = crate::service::conjugatepoc_inherent_data_providers($author)?;

		let builder = substrate_service::ServiceBuilder::new_full::<
			conjugatepoc_runtime::opaque::Block, conjugatepoc_runtime::RuntimeApi, crate::service::Executor
		>($config)?
			...
			.with_import_queue(|_config, client, select_chain, _transaction_pool| {
				let import_queue = consensus_poc::import_queue(
					Box::new(client.clone()),
					client.clone(),
					conjugatepoc_poc::RandomXAlgorithm::new(client.clone()),
					0,
					select_chain,
	...
	-> Result<impl AbstractService, ServiceError>
{
	...
	if is_authority {
		for _ in 0..threads {
			...
			consensus_poc::start_mine(
				Box::new(service.client().clone()),
				service.client(),
				conjugatepoc_poc::RandomXAlgorithm::new(service.client()),
				proposer,
				None,
				round,
				service.network(),
...
pub fn new_light<C: Send + Default + 'static>(config: Configuration<C, GenesisConfig>, author: Option<&str>)
	-> Result<impl AbstractService, ServiceError>
{
	let inherent_data_providers = conjugatepoc_inherent_data_providers(author)?;

...
impl Alternative {
	...
	pub(crate) fn load(self) -> Result<ChainSpec, String> {
		Ok(match self {
			...
			Alternative::Conjugatepoc => ChainSpec::from_genesis(
				"Conjugatepoc",
				"conjugatepoc",
				|| mainnet_genesis(),
				vec![
	...
	pub(crate) fn from(s: &str) -> Option<Self> {
		match s {
			...
			"" | "conjugatepoc" => Some(Alternative::Conjugatepoc),
			_ => None,
		}
	}
}
...
fn mainnet_genesis() -> GenesisConfig {
	GenesisConfig {
		system: Some(SystemConfig {
			code: include_bytes!("../res/0-genesis/conjugatepoc_runtime.compact.wasm").to_vec(),
			changes_trie_config: Default::default(),
		}),
		indices: Some(IndicesConfig {
			ids: vec![],
